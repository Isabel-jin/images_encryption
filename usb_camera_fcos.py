...
def imgRestore(image, bboxes, gt_classes_index=None, classes=get_classes()):
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if frame.shape[0]!=disp_h and frame.shape[1]!=disp_w:
            frame = cv2.resize(frame, (disp_w,disp_h), interpolation=cv2.INTER_AREA)

        # Draw bboxs
        box_bgr = imgRestore(frame, prediction_bbox)
